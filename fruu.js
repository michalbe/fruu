...
Emitter.prototype.removeAllParticles = function () {
	for (var i = 0; i < this.particles.length; i++) this.particles[i].dead = true;
};
/**
...
class Presentation {
  ...
  change_slide(dir) {
    if (this.current_slide + dir === -1 || this.current_slide + dir === this.slides.data.length) {
      return;
    }
  ...
  render_slide() {
    ...
    if (typeof this.slides.data[this.current_slide] === 'string') {
      ...
      this.context.fillText(this.slides.data[this.current_slide], window.innerWidth / 2, window.innerHeight / 2 + 50, window.innerWidth * 0.95);

      const image_data = this.context.getImageData(0, 0, window.innerWidth, window.innerHeight);

      this.text_init = this.emitter.addInitialize(new Proton.P(new Proton.ImageZone(image_data, 0, 50)));

      if (this.random_behaviour && this.gravity) {
        this.random_behaviour.reset(2, 2, .2);
        this.gravity.reset(0);
      }
    } else if (typeof this.slides.data[this.current_slide] === 'object' && this.slides.data[this.current_slide].image) {

      this.color_behaviour = this.emitter.addBehaviour(new Proton.Color(this.slides.data[this.current_slide].color || defaults.color));
